...
/**
 * The twelve live builds as a card deck. One open card in front (screenshot
 * plus a paper info panel), the rest fanned out behind it on both sides with
 * the client name on a coloured spine. Any spine, the arrows, the arrow keys
 * or a swipe brings another build to the front. Cards stay mounted and only
 * change transforms, so every move is one smooth transition.
 */
            ...
            >
              ...
              <div className="deck-info">
                <div className="deck-info-inner">
                  <span className="font-mono text-[11px] tracking-[.2em] text-muted-2">
                    {pad(i + 1)} / {pad(N)}
                  </span>
                  <h3 className="display m-0 mt-5 text-[clamp(34px,3.2vw,46px)] leading-[.92] text-ink">{w.name}</h3>
                  <p className="m-0 mt-4 text-[15px] leading-[1.55] text-muted-2">{w.built}</p>
                  <a
                    href={w.url}
                    target="_blank"
                    ...
                    data-magnetic=""
                    className={cn(
                      "mt-6 inline-flex w-fit items-center gap-2 rounded-full bg-ink px-5 py-3 font-brand text-[15px] font-semibold text-accent transition-colors hover:bg-ink-3",
                      focus,
                    )}
                  >
                    Visit the site <span aria-hidden="true">↗</span>
                  </a>
                  <span className="mt-auto flex items-center justify-between pt-6 font-mono text-[11px] tracking-[.14em] text-ink">
                    <span>{w.location ? w.location.toUpperCase() : "LIVE"}</span>
                    <span className="text-muted-2">{new URL(w.url).hostname.replace(/^www\./, "")}</span>
                  </span>
                </div>
              </div>
            </article>
